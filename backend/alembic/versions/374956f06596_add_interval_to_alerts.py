...
def upgrade() -> None:
    ...
    # Use checkfirst=True to avoid errors if objects don't exist (fresh DB)
    from sqlalchemy import inspect

    conn = op.get_bind()
    inspector = inspect(conn)

    # Drop 'user' table if exists (legacy auth)
    if inspector.has_table('user'):
        if inspector.has_index('user', 'ix_user_email'):
            op.drop_index('ix_user_email', table_name='user')
        if inspector.has_index('user', 'ix_user_id'):
            op.drop_index('ix_user_id', table_name='user')
        op.drop_table('user')

    # Drop 'alerttrigger' table if exists (renamed to alert_trigger)
    if inspector.has_table('alerttrigger'):
        if inspector.has_index('alerttrigger', 'ix_alerttrigger_alert_id'):
            op.drop_index('ix_alerttrigger_alert_id', table_name='alerttrigger')
        if inspector.has_index('alerttrigger', 'ix_alerttrigger_id'):
            op.drop_index('ix_alerttrigger_id', table_name='alerttrigger')
        if inspector.has_index('alerttrigger', 'ix_alerttrigger_triggered_at'):
