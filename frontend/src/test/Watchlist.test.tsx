...
describe('Watchlist Rendering and Empty State', () => {
  ...
  it('shows empty state message when watchlist is empty', async () => {
    ...
    const { rerender } = render(<Watchlist items={[]} onAddClick={vi.fn()} onRemove={vi.fn()} onSelect={vi.fn()} />)
  ...
  it('flashes green when price increases', async () => {
    ...
    const { rerender } = render(<Watchlist items={items} onAddClick={vi.fn()} onRemove={vi.fn()} onSelect={vi.fn()} />)
    
    // Update price
    const updatedItems = [{ symbol: 'IBM', price: 146.00, change: 2.30, changePercent: 1.60 }]
    rerender(<Watchlist items={updatedItems} onAddClick={vi.fn()} onRemove={vi.fn()} onSelect={vi.fn()} />)
  ...
  it('calls onReorder when items are dragged', async () => {
    ...
    render(<Watchlist items={items} onAddClick={vi.fn()} onRemove={vi.fn()} onSelect={vi.fn()} onReorder={onReorder} />)
  ...
  it('shows context menu on right click', async () => {
    const items = [{ symbol: 'IBM', price: 145.20, change: 1.50, changePercent: 1.04 }]
    render(<Watchlist items={items} onAddClick={vi.fn()} onRemove={vi.fn()} onSelect={vi.fn()} onReorder={vi.fn()} />)
  ...
  it('enters selection mode and selects multiple items', async () => {
    ...
    render(<Watchlist items={items} onAddClick={vi.fn()} onRemove={onRemove} onSelect={vi.fn()} onReorder={vi.fn()} />)
